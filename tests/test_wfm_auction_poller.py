...
def _auc(aid: str, price: int) -> dict:
    return {
        ...
        "top_bid": None,
        "item": {
            "weapon_url_name": "tonkor", "polarity": "vazarin",
            "mod_rank": 8, "re_rolls": 0,
...
async def test_tick_writes_snapshot_per_tier(repo: Repo) -> None:
    ...
    for tier in ("god", "mid", "low", "all"):
        rows = await repo.riven_snapshot_history(weapon_slug="tonkor", tier=tier, since_ts=now - 10)
        assert len(rows) == 1, f"no snapshot for tier {tier}"


@pytest.mark.asyncio
async def test_tick_upserts_active_auctions_then_marks_disappeared_gone(repo: Repo) -> None:
    now = int(time.time())
    await repo.add_riven_watch("tonkor", ts=now)
    client = AsyncMock()
    publisher = AsyncMock()
    ...
    client.get_riven_auctions = AsyncMock(return_value=[_auc("a1", 100), _auc("a2", 200), _auc("a3", 300)])
    poller = AuctionPoller(repo=repo, client=client, publisher=publisher)
    await poller.tick(now=now)
    ...
    client.get_riven_auctions = AsyncMock(return_value=[_auc("a1", 100), _auc("a3", 300), _auc("a4", 400)])
...
async def test_tick_publishes_outliers_when_history_exists(repo: Repo) -> None:
    ...
    for offset in range(1, 15):
        await repo.write_riven_snapshot(
            weapon_slug="tonkor", ts=now - offset * 3600, tier="mid",
            count=10, min_price=400, p25=450, median=500, p75=550, max_price=700,
        )
    ...
    aucs = [_auc(f"a{i}", price=p) for i, p in enumerate([200, 220, 250, 280, 300, 320, 400, 500, 600, 700, 800, 1000])]
    client = AsyncMock()
    client.get_riven_auctions = AsyncMock(return_value=aucs)
    publisher = AsyncMock()
    publisher.publish = AsyncMock()
...
async def test_tick_swallows_wfm_error(repo: Repo) -> None:
    ...
    rows = await repo.riven_snapshot_history(weapon_slug="kuva_bramma", tier="all", since_ts=now - 10)
    assert len(rows) == 1
