...
def test_higher_roll_value_grades_higher() -> None:
    hi = grade_roll_value(
        "critical_damage", 180, stat_count=2, has_negative=False, disposition=1.0
    )
    lo = grade_roll_value(
        "critical_damage", 90, stat_count=2, has_negative=False, disposition=1.0
    )
    assert hi > lo
...
def test_roll_value_saturates_at_ceiling() -> None:
    ...
    assert (
        grade_roll_value(
            "critical_chance", 999, stat_count=2, has_negative=False, disposition=1.0
        )
        == 1.0
    )
...
def test_unknown_stat_gets_full_presence_credit() -> None:
    ...
    assert (
        grade_roll_value(
            "damage_vs_grineer", 45, stat_count=2, has_negative=False, disposition=1.0
        )
        == 1.0
    )
...
def test_three_positive_roll_has_lower_ceiling_than_two() -> None:
    ...
    two = grade_roll_value(
        "critical_damage", 90, stat_count=2, has_negative=False, disposition=1.0
    )
    three = grade_roll_value(
        "critical_damage", 90, stat_count=3, has_negative=False, disposition=1.0
    )
    assert three > two
...
def test_negative_raises_ceiling() -> None:
    ...
    # fraction of it.
    no_neg = grade_roll_value(
        "critical_damage", 90, stat_count=2, has_negative=False, disposition=1.0
    )
    with_neg = grade_roll_value(
        "critical_damage", 90, stat_count=2, has_negative=True, disposition=1.0
    )
...
def test_low_disposition_grades_a_fixed_value_higher() -> None:
    ...
    low = grade_roll_value(
        "critical_damage", 90, stat_count=2, has_negative=False, disposition=0.65
    )
    high = grade_roll_value(
        "critical_damage", 90, stat_count=2, has_negative=False, disposition=1.3
    )
    assert low > high
...
def test_negative_penalty_full_weight_for_archetype_fatal() -> None:
    ...
    assert negative_penalty("critical_chance", crit) > negative_penalty(
        "critical_chance", status
    )
...
def test_dead_negative_on_god_roll_barely_dents_grade() -> None:
    ...
    assert (
        score_riven(god, crit).headline.grade
        == score_riven(with_zoom, crit).headline.grade
    )
