...
def test_stale_slugs_filters_by_age() -> None:
    ...
    assert store.stale_slugs(["fresh", "old", "missing"], max_age=10.0, now=now) == {
        "old",
        "missing",
    }
...
def test_set_overwrites_previous() -> None:
    store = PriceStore()
    s1 = PriceStats(
        slug="x", sell_min=10, sell_median=12, sell_spread=2, buy_max=8, fetched_at=1.0
    )
    s2 = PriceStats(
        slug="x", sell_min=20, sell_median=22, sell_spread=2, buy_max=18, fetched_at=2.0
    )
    store.set(s1)
    store.set(s2)
