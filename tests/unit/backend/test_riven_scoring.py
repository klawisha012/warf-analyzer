...
def _profile(critness: float, statusness: float, **stats) -> Profile:
    return Profile(
        kind="base",
        critness=critness,
        statusness=statusness,
        stats=stats,
        omega_attenuation=1.0,
    )
...
def test_build_profiles_base_only_in_m1() -> None:
    profiles = build_profiles(
        _weapon_row("Kuva Bramma", crit_chance=0.5, omega_attenuation=0.65)
    )
...
def test_score_riven_cc_riven_scores_high_on_crit_low_on_status() -> None:
    ...
    crit = build_profiles(
        _weapon_row(
            "Kuva Bramma", crit_chance=0.5, status_chance=0.1, omega_attenuation=0.65
        )
    )
    status = build_profiles(
        _weapon_row(
            "Kuva Nukor", crit_chance=0.1, status_chance=0.4, omega_attenuation=1.0
        )
    )
...
def test_score_riven_negative_on_archetype_stat_hurts_more_than_utility() -> None:
    crit = build_profiles(
        _weapon_row("Dread", crit_chance=0.5, status_chance=0.1, omega_attenuation=1.0)
    )
    ...
    assert (
        score_riven(with_fatal_neg, crit).headline.score < base
    )  # archetype neg hurts
...
def test_raw_damage_weapon_can_reach_top_grade() -> None:
    ...
    raw = build_profiles(
        _weapon_row("Some Launcher", crit_chance=0.0, status_chance=0.0)
    )
...
def test_multishot_scores_lower_on_launcher_than_rifle() -> None:
    rifle = build_profiles(
        _weapon_row("Rifle", crit_chance=0.0, status_chance=0.0, type="Rifle")
    )
    launcher = build_profiles(
        _weapon_row("Launcher", crit_chance=0.0, status_chance=0.0, type="Launcher")
    )
    riven = [_pos("multishot")]
    assert (
        score_riven(riven, launcher).headline.score
        < score_riven(riven, rifle).headline.score
    )
...
def test_all_negative_roll_floors_at_f() -> None:
    ...
    assert (
        res.unscored is False and res.headline.score == 0 and res.headline.grade == "F"
    )
...
def test_market_signal_steal_when_good_grade_priced_below_median() -> None:
    ...
    assert (
        classify_market_signal("A", buyout_price=96, median=120) == "steal"
    )  # exactly 0.8x boundary
...
def test_market_signal_trap_when_bad_grade_priced_above_median() -> None:
    ...
    assert (
        classify_market_signal("F", buyout_price=150, median=120) == "trap"
    )  # exactly 1.25x boundary
...
def test_market_signal_needs_margin_not_one_plat() -> None:
    ...
    assert (
        classify_market_signal("F", buyout_price=130, median=120) is None
    )  # over but under 1.25x
...
def test_market_signal_none_for_fair_or_missing_inputs() -> None:
    ...
    assert (
        classify_market_signal("F", buyout_price=10, median=120) is None
    )  # junk but cheap = fine
    assert classify_market_signal(None, buyout_price=10, median=120) is None  # unscored
    assert (
        classify_market_signal("S", buyout_price=None, median=120) is None
    )  # no price
    assert (
        classify_market_signal("S", buyout_price=-50, median=120) is None
    )  # negative price guard
    assert (
        classify_market_signal("S", buyout_price=10, median=None) is None
    )  # no median
    assert (
        classify_market_signal("S", buyout_price=10, median=0) is None
    )  # zero median guard
