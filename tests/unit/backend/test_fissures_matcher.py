...
def test_mission_filter() -> None:
    assert (
        matches(_fissure(mission_type="Survival"), _sub(mission_type="Survival"))
        is True
    )
    assert (
        matches(_fissure(mission_type="Survival"), _sub(mission_type="Capture"))
        is False
    )
...
def test_combined_filter_all_must_match() -> None:
    ...
    assert (
        matches(
            f, _sub(era="Axi", mission_type="Survival", is_hard=False, is_storm=False)
        )
        is True
    )
