    ...
    r.load(
        [
            ...
            ItemRef(
                ...
                item_name="Mag Prime Neuroptics BP",
                thumb_url=None,
                vaulted=False,
                wfm_id="3",
            ),
            ItemRef(
                ...
                item_name="Mag Prime Chassis BP",
                thumb_url=None,
                vaulted=False,
                wfm_id="4",
            ),
            ItemRef(
                ...
                item_name="Mag Prime Systems BP",
                thumb_url=None,
                vaulted=False,
                wfm_id="5",
            ),
...
def test_loader_builds_mag_prime_set(tmp_path: Path, resolver: SlugResolver) -> None:
    ...
    cached_dir = tmp_path
    (cached_dir / "Warframes.json").write_text(
        fixture.read_text(encoding="utf-8"), encoding="utf-8"
    )

    sets = load_set_compositions_from_aleca(
        cached_json_dir=cached_dir, resolver=resolver
    )
...
def test_loader_skips_items_without_components(
    tmp_path: Path, resolver: SlugResolver
) -> None:
...
def test_loader_skips_unresolvable_components(
    tmp_path: Path, resolver: SlugResolver
) -> None:
    ...
    (tmp_path / "Warframes.json").write_text(
        json.dumps(
            [
                {
                    "name": "Mystery Prime",
                    "uniqueName": "/Lotus/Powersuits/Mystery/MysteryPrime",
                    "components": [
                        {
                            "uniqueName": "/Lotus/Types/Recipes/WarframeRecipes/MysteryPrimeBlueprint",
                            "itemCount": 1,
                        }
                    ],
                }
