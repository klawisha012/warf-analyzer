...
async def test_write_snapshot_writes_4_rows(repo: Repo) -> None:
    ...
    sells_online = await repo.history(
        slug="kronen_prime_blade", side="sell", online_only=1, since_ts=ts - 60
    )
    sells_all = await repo.history(
        slug="kronen_prime_blade", side="sell", online_only=0, since_ts=ts - 60
    )
    buys_online = await repo.history(
        slug="kronen_prime_blade", side="buy", online_only=1, since_ts=ts - 60
    )
    buys_all = await repo.history(
        slug="kronen_prime_blade", side="buy", online_only=0, since_ts=ts - 60
    )
