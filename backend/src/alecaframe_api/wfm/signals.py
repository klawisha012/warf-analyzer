...
class SignalContext:
    ...
    history_7d: list[
        Snapshot
    ]  # sell-side, online-only, last 7 days, newest first or oldest first — caller's choice
    ...
    set_context: dict[str, Any] | None = (
        None  # {set_slug, parts_cost, set_price} when applicable
    )
...
def overpriced_mine(ctx: SignalContext) -> SignalEvent | None:
    if (
        ctx.my_listing_price is None
        or ctx.current_sell is None
        or not ctx.current_sell.top5
    ):
        return None
...
def competitor_undercut(ctx: SignalContext) -> SignalEvent | None:
    ...
    if ctx.current_sell.top5[0] < ctx.my_listing_price - 1:
        return SignalEvent(
            slug=ctx.slug,
            ts=ctx.now_ts,
            signal_type="competitor_undercut",
...
def floor_drop(ctx: SignalContext) -> SignalEvent | None:
    ...
    if drop_pct >= 0.10:
        return SignalEvent(
            slug=ctx.slug,
            ts=ctx.now_ts,
            signal_type="floor_drop",
            payload={
                "current": ctx.current_sell.min_price,
                "baseline": baseline,
...
def volume_spike(ctx: SignalContext) -> SignalEvent | None:
    ...
    if ctx.current_sell.volume_qty > 3 * mean_vol:
        return SignalEvent(
            ...
            payload={
                "current_volume": ctx.current_sell.volume_qty,
...
def vault_premium(ctx: SignalContext) -> SignalEvent | None:
    if (
        not ctx.is_vaulted
        or ctx.current_sell is None
        or ctx.current_sell.median is None
    ):
        return None
    medians = _medians(ctx.history_7d)
    if len(medians) < 5:
    ...
    if ctx.current_sell.median > baseline * 1.5:
        return SignalEvent(
            slug=ctx.slug,
            ts=ctx.now_ts,
            signal_type="vault_premium",
            payload={
                "current_median": ctx.current_sell.median,
