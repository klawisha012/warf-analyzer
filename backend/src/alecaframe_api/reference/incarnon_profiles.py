...
class IncarnonProfile(BaseModel):
    ...
    weapon_type: str = Field(min_length=1)        # for multishot modulation (Launcher/Rifle/...)
...
def _p(name: str, wtype: str, cc: float, cd: float, sc: float, slug: str) -> IncarnonProfile:
    return IncarnonProfile(
        weapon_name=name, weapon_type=wtype,
        crit_chance=cc, crit_damage=cd, status_chance=sc,
        evolution_build=_BASE,
        source_url=f"https://wiki.warframe.com/w/{slug}",
        entered_date=_TODAY, game_version=CURRENT_GAME_VERSION,
    )
...
KNOWN_INCARNON_WEAPONS: frozenset[str] = frozenset(
    normalize_name(n) for n in (
        "Braton", "Lato", "Paris", "Kunai",
        "Boar", "Gammacor", "Angstrum", "Gorgon",
        "Latron", "Furis", "Strun",
        "Lex", "Boltor", "Bronco",
        "Torid", "Dual Toxocyst", "Miter", "Atomos",
        "Soma", "Vasto", "Burston",
        "Zylok", "Dread", "Despair",
        "Dera", "Sybaris", "Cestra", "Sicarus",
        "Vectis", "Stug", "Ballistica",
    )
)
