...
export const keys = {
  ...
  rivenHistory: (slug: string, tier: string, days: number) =>
    ["rivens", "history", slug, tier, days] as const,
...
export const fetchers = {
  ...
  wfmOrders: (slug: string, includeOffline: boolean) =>
    api<OrderBookResponse>(
      `/wfm/orders/${encodeURIComponent(slug)}?include_offline=${includeOffline ? 1 : 0}`,
    ),
  ...
  refItems: (category: string | null) =>
    api<ItemBaseStatsListResponse>(
      `/reference/items${category ? `?category=${encodeURIComponent(category)}` : ""}`,
    ),
  ...
  rivenAuctions: (slug: string, fresh = false) =>
    api<RivenAuctionsResponse>(
      `/rivens/auctions/${encodeURIComponent(slug)}${fresh ? "?fresh=1" : ""}`,
    ),
  ...
  rivenHistory: (slug: string, tier = "all", days = 7) =>
    api<RivenHistoryResponse>(
      `/rivens/history/${encodeURIComponent(slug)}?tier=${tier}&days=${days}`,
    ),
  ...
  fissuresTest: () =>
    api<{ sent: number; chats: number }>("/fissures/telegram/test", { method: "POST" }),
};
