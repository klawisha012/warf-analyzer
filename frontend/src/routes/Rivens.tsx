            ...
            >
              {(opt) => (
                <button
                  type="button"
                  class="seg-btn"
                  classList={{ active: p.lens === opt.v }}
                  onClick={() => p.setLens(opt.v)}
                >
                  {opt.label}
                </button>
              )}
...
function PaginatedAuctionTable(props: {
  rows: RivenAuctionRow[];
...
function GradeCell(props: { row: RivenAuctionRow; lens: ProfileLens }) {
  ...
  const primary = createMemo(
    () => (props.lens === "incarnon" ? inc() : base()) ?? base() ?? inc() ?? headline(),
  );
  ...
  return (
    <Show
      ...
      fallback={
        <span class="text-dim" title={props.row.unscored_reason ?? ""}>
          —
        </span>
      }
    >
      <span class="font-bold" classList={gradeColorClass(primary()!.grade)}>
        {primary()!.grade}
      </span>
      ...
      <Show when={secondary()}>
        <span class="text-[10px] text-dim">
          {" "}
          ({props.lens === "incarnon" ? t("rivens.lensBase") : t("rivens.lensIncarnon")}{" "}
          {secondary()!.grade})
        </span>
      </Show>
                  ...
                  <span class="flex flex-wrap gap-1">
                    <For each={r.attributes}>
                      {(a) => (
                        ...
                        >
                          {rivenAttrName(a.name)} {a.value > 0 && a.positive ? "+" : ""}
                          {a.value}
                        </span>
                      )}
                    </For>
